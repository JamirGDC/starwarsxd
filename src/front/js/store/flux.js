...
const getState = ({ getStore, getActions, setStore }) => {
	return {
		...
		actions: {
			...
			getPeopleStar: async (producto_id) => {
				const store = getStore();
				try {
					const response = await fetch(`https://www.swapi.tech/api/people/${producto_id}`);
					if (!response.ok) {
						throw new Error(`HTTP error! status: ${response.status}`);
					}
					const data = await response.json();
					console.log("Detalles del producto recibidos:", data);
					setStore({ ...store, productDetails: data.result });
					localStorage.setItem('productDetails', JSON.stringify(data.result));
				} catch (error) {
					console.error("Error loading product details:", error);
				}
			},
			getPlanetDetail: async (producto_id) => {
				const store = getStore();
				try {
					const response = await fetch(`https://www.swapi.tech/api/planets/${producto_id}`);
					if (!response.ok) {
						throw new Error(`HTTP error! status: ${response.status}`);
					}
					const data = await response.json();
					console.log("Detalles del producto recibidos:", data);
					setStore({ ...store, planetdetail: data.result });
					localStorage.setItem('planetdetail', JSON.stringify(data.result));
				} catch (error) {
					console.error("Error loading product details:", error);
				}
			},
